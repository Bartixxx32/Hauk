...
public abstract class SessionManager {
  ...
  /**
   * A callback instance that is called upon when sharing is stopped, to reset
   * the UI.
   */
  private final StopSharingCallback stopCallback;

  /**
   * Intent for the location pusher, so that it can be stopped if already
   * running when launching the app.
   */
  ...
  /**
   * A list of GNSS status update listeners that implementors have attached to
   * this session manager instance.
   */
  private final List<GNSSStatusUpdateListener> upstreamUpdateHandlers;

  /**
   * A list of share creation and removal listeners that implementors have
   * attached to this session manager instance.
   */
  private final List<ShareListener> upstreamShareListeners;

  /**
   * A list of session creation listeners that implementors have attached to
   * this session manager instance.
   */
  private final List<SessionListener> upstreamSessionListeners;

  /**
   * A list of shares that this session manager knows about. This is a mapping
   * between share IDs and the corresponding {@link Share} instances.
   */
  private final Map<String, Share> knownShares;

  /**
   * The currently active session. Used to check if a session has to be
   * initialized when starting a new share.
   */
   ...
   * @param stopCallback A callback that is called when sharing sessions are
   *     stopped.
   */
  protected SessionManager(Context ctx, StopSharingCallback stopCallback) {
    ...
    this.stopTask = new StopSharingTask(this.ctx, this.stopCallback) {
      @Override
      public void cleanup() {
        // Called when sharing ends. Clear the active session, and all
        // collections of active shares present in this class, then propagate
        // this stop message upstream to all session listeners.
        Log.d(
            "Performing stop task cleanup for task %s and stopping timed callback on handler %s",
            this, SessionManager.this.handler); // NON-NLS
        ...
        for (GNSSStatusUpdateListener listener :
             SessionManager.this.upstreamUpdateHandlers) {
          listener.onShutdown();
        }
      }
    };
   ...
   */
  public final boolean isSessionActive() { return this.stopTask.canExecute(); }

  /**
   * Request that the current sharing session is stopped.
   */
  public final void stopSharing() { this.stopTask.run(); }

  /**
   * Attaches a GNSS status listener to the session manager. This listener is
   * called whenever the GNSS/location status updates.
   *
  ...
  /**
   * Attaches a share change listener to the session manager. This listener is
   * called whenever a share is started or stopped.
   *
   * @param listener The listener to attach.
   */
  ...
  /**
   * Attaches a session creation listener to the session manager. This listener
   * is called whenever a session is created or fails to be created due to
   * missing permissions.
   *
   * @param listener The listener to attach.
   */
   ...
   *
   * @param prompt A callback to prompt the user for whether or not they want to
   *     resume shares if
   *               any are found in storage.
   */
  public final void resumeShares(ResumePrompt prompt) {
    // Check if the location push service is already running. This will happen
    // if the main UI activity is killed/stopped, but the app itself and the
    // pushing service keeps running in the background. If this happens, the
    // push service should be silently restarted to ensure it behaves properly
    // with new instances of GNSSActiveHandler and StopSharingTask that will be
    // created and attached when creating a new SessionManager in MainActivity.
    // There is probably a cleaner way to do this.
    if (pusher != null) {
      Log.d(
          "Pusher is non-null (%s), stopping and nulling it and calling service relauncher",
          pusher); // NON-NLS
      this.ctx.stopService(pusher);
      pusher = null;
      this.resumable.tryResumeShare(
          new ServiceRelauncher(this, this.resumable));
    } else {
      Log.d("Pusher is null, calling resumption prompter"); // NON-NLS
      this.resumable.tryResumeShare(
          new AutoResumptionPrompter(this, this.resumable, prompt));
    }
  }
  ...
  public final void resumeShares() {
    if (pusher != null) {
      Log.d(
          "Pusher is non-null (%s), stopping and nulling it before calling service relauncher",
          pusher); // NON-NLS
      this.ctx.stopService(pusher);
      pusher = null;
    }
  ...
  /**
   * A preparation step for initiating sessions. Checks location services status
   * and instantiates a response handler for the session initiation packet.
   *
   * @param upstreamCallback An upstream callback to receive initiation progress
   *     updates.
   * @return A response handler for use with the {@link
   *     SessionInitiationPacket}.
   * @throws LocationServicesDisabledException if location services are
   *     disabled.
   * @throws LocationPermissionsNotGrantedException if location permissions have
   *     not been granted.
   */
  private SessionInitiationPacket.ResponseHandler
  preSessionInitiation(final SessionInitiationResponseHandler upstreamCallback,
                       final SessionInitiationReason reason)
      throws LocationServicesDisabledException,
             LocationPermissionsNotGrantedException {
    // Check for location permission and prompt the user if missing. This
    // returns because the checking function creates async dialogs here - the
    // user is prompted to press the button again instead.
    ...
    boolean gpsDisabled = locMan != null && !locMan.isProviderEnabled(
    ...
    // Create a handler for our request to initiate a new session. This is
    // declared separately from the SessionInitiationPackets below to avoid code
    // duplication.
    ...
    return new SessionInitiationPacket.ResponseHandler() {
      ...
      public void onSessionInitiated(Share share) {
        Log.i("Session was initiated for share %s; setting session resumable",
              share); // NON-NLS
      ...
      @Override
      public void onShareModeIncompatible(ShareMode downgradeTo,
                                          Version backendVersion) {
        Log.e(
            "The requested sharing mode is incompatible because the server is out of date (backend=%s)",
            backendVersion); // NON-NLS
        upstreamCallback.onShareModeForciblyDowngraded(downgradeTo,
                                                       backendVersion);
      }
      ...
      public void onE2EUnavailable(Version backendVersion) {
        Log.e(
            "End-to-end encryption was requested but dropped because the server is out of date (backend=%s)",
            backendVersion); // NON-NLS
   ...
   *
   * @param initParams       Connection parameters describing the backend server
   *     to connect to.
   * @param upstreamCallback An upstream callback to receive initiation progress
   *     updates.
   ...
   */
  public final void
  shareLocation(SessionInitiationPacket.InitParameters initParams,
                SessionInitiationResponseHandler upstreamCallback,
                AdoptabilityPreference allowAdoption)
      throws LocationPermissionsNotGrantedException,
             LocationServicesDisabledException {
    SessionInitiationPacket.ResponseHandler handler = preSessionInitiation(
        upstreamCallback, SessionInitiationReason.USER_STARTED);

    // Create a handshake request and handle the response. The handshake
    // transmits the duration and interval to the server and waits for the
    // server to return a session ID to confirm session creation.
    Log.i("Creating single-user session initiation packet"); // NON-NLS
    new SessionInitiationPacket(this.ctx, initParams, handler, allowAdoption)
        .send();
  }
  ...
  shareLocation(SessionInitiationPacket.InitParameters initParams,
                SessionInitiationResponseHandler upstreamCallback,
                String nickname) throws LocationPermissionsNotGrantedException,
                                        LocationServicesDisabledException {
    SessionInitiationPacket.ResponseHandler handler = preSessionInitiation(
        upstreamCallback, SessionInitiationReason.USER_STARTED);
  ...
  shareLocation(SessionInitiationPacket.InitParameters initParams,
                SessionInitiationResponseHandler upstreamCallback,
                String nickname, String groupPin)
      throws LocationPermissionsNotGrantedException,
             LocationServicesDisabledException {
    SessionInitiationPacket.ResponseHandler handler = preSessionInitiation(
        upstreamCallback, SessionInitiationReason.USER_STARTED);
    ...
    new SessionInitiationPacket(this.ctx, initParams, handler, nickname,
                                groupPin)
        .send();
  }

  /**
   * Executes a location sharing session against the server. This can be a new
  ...
  public final void stopSharing(final Share share) {
    new StopSharingPacket(this.ctx, share) {
      ...
      public void onSuccess() {
        ...
        for (ShareListener listener :
             SessionManager.this.upstreamShareListeners) {
  ...
  /**
   * For internal use only. Spawns a new location push service that actually
  ...
  private void initiateSessionForExistingShare(Share share,
                                               SessionInitiationReason reason) {
    ...
    if (this.ctx.checkSelfPermission(
            Manifest.permission.ACCESS_FINE_LOCATION) ==
        PackageManager.PERMISSION_GRANTED) {
      Log.i(
          "Location permission has been granted; sharing will commence"); // NON-NLS
      GNSSActiveHandler statusUpdateHandler =
          new GNSSStatusUpdateTask(share.getSession());
      ...
      pusher.putExtra(Constants.EXTRA_SHARE,
                      ReceiverDataRegistry.register(share));
      pusher.putExtra(Constants.EXTRA_STOP_TASK,
                      ReceiverDataRegistry.register(this.stopTask));
      pusher.putExtra(Constants.EXTRA_HANDLER,
                      ReceiverDataRegistry.register(this.handler));
      pusher.putExtra(Constants.EXTRA_GNSS_ACTIVE_TASK,
                      ReceiverDataRegistry.register(statusUpdateHandler));

      // Android O and higher require the service to be started as a foreground
      // service for it not to be killed.
      ...
      Log.d("Setting static pusher %s (was %s)", pusher,
            SessionManager.pusher); // NON-NLS
      ...
      Log.i(
          "Scheduling session task %s for expiration in %s milliseconds on handler %s",
          this.stopTask, expireIn, this.handler); // NON-NLS
      ...
      Log.w(
          "Location permission has not been granted; sharing will not commence"); // NON-NLS
      for (SessionListener listener : this.upstreamSessionListeners) {
  ...
  /**
   * Checks whether or not the user granted Hauk permission to use their device
   * location. If permission has not been granted, this function creates a
   * dialog which runs asynchronously, meaning this function does not wait until
   * permission has been granted before it returns.
   *
  ...
  private boolean hasLocationPermission() {
    if (this.ctx.checkSelfPermission(
        ...
        PackageManager.PERMISSION_GRANTED) {
      Log.w(
          "Location permission has not been granted. Asking user for permission"); // NON-NLS
  ...
  /**
   * The GNSS status handler that the {@link SessionManager} itself uses to
   * receive status updates from the GNSS listeners. Used to propagate events
   * further upstream.
   */
  private final class GNSSStatusUpdateTask implements GNSSActiveHandler {
    ...
    private GNSSStatusUpdateTask(Session session) { this.session = session; }
    ...
    public void onShareListReceived(String linkFormat, String[] shareIDs) {
      ...
      for (int i = 0; i < currentShares.size(); i++) {
        ...
        if (!SessionManager.this.knownShares.containsKey(shareID)) {
          ...
          // of shares that can be initiated by a remote user (through
          // adoption).
          Share newShare =
              new Share(this.session, String.format(linkFormat, shareID),
                        shareID, ShareMode.JOIN_GROUP);
          Log.i("Received unknown share %s from server", newShare); // NON-NLS
          shareLocation(newShare, SessionInitiationReason.SHARE_ADDED);
        }
      }
      for (Iterator<Map.Entry<String, Share>> it =
               SessionManager.this.knownShares.entrySet().iterator();
           it.hasNext();) {
        Map.Entry<String, Share> entry = it.next();
        if (!currentShares.contains(entry.getKey())) {
          // A share has been removed.
          Log.i("Share %s was terminated on server, removing",
                entry.getKey()); // NON-NLS
          it.remove();
